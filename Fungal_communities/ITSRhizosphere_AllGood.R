...
ps_rarefiedRhizosphere <- rarefy_even_depth(ps_Rhizosphere, sample.size =   33404 , replace = FALSE, trimOTUs = TRUE, verbose = TRUE)
# Allocation of objects after rarefying
asv_counts_rarefiedRZ <- as.matrix(otu_table(ps_rarefiedRhizosphere))
After_rar_sequence_readsRZ <- sum(asv_counts_rarefiedRZ)
After_rar_sequence_readsRZ
# Taxa
after_rar_num_asvsRZ<- ntaxa(ps_rarefiedRhizosphere)
after_rar_num_asvsRZ
# Check
summarize_phyloseq(ps_rarefiedRhizosphere)
ps_rarefiedRhizosphere
# Check the structure
str(sample_data(ps_rarefiedRhizosphere))
...
rank_names(ps_rarefiedRhizosphere)
...
for (rank in taxonomic_ranks) {
  glommed_ps <- ps_rarefiedRhizosphere %>%
...
otu_counts <- as.matrix(otu_table(ps_rarefiedRhizosphere))
total_sequence_reads <- sum(otu_counts)
total_sequence_reads
num_asvs <- ntaxa(ps_rarefiedRhizosphere)
num_asvs

#### Number of sequences and ASVs for each veg species ####
# Acacia mangiumY: of 2 years old
ps_RhizoAcaciaY <- subset_samples(ps_rarefiedRhizosphere, Rhizosphere == "Acacia mangiumY")
ps_RhizoAcaciaY
...
ps_RhizoAcaciaO <- subset_samples(ps_rarefiedRhizosphere, Rhizosphere == "Acacia mangiumO")
ps_RhizoAcaciaO
...
# Saccharum spp
ps_RhizoSac <- subset_samples(ps_rarefiedRhizosphere, Rhizosphere == "Saccharum spp.")
ps_RhizoSac
...
ps_RhizoMah <- subset_samples(ps_rarefiedRhizosphere, Rhizosphere == "Mahogany (Swietenia macrophylla)")
ps_RhizoMah
...
ps_RhizoNarra <- subset_samples(ps_rarefiedRhizosphere, Rhizosphere == "Narra (Pterocarpus indicus)")
ps_RhizoNarra
...
# Coccus nucifera
ps_RhizoCoccus <- subset_samples(ps_rarefiedRhizosphere, Rhizosphere == "Coccus nucifera")
ps_RhizoCoccus
...
# Imperata cylindrica
ps_RhizoImper <- subset_samples(ps_rarefiedRhizosphere, Rhizosphere == "Imperata cylindrica")
ps_RhizoImper
...
meco_datasetRZ <- phyloseq2meco(ps_rarefiedRhizosphere)
# check
meco_datasetRZ
...
meco_datasetRZ$tax_table %<>% base::subset(Kingdom == "k__Fungi")
...
# See levels
meco_datasetRZ$sample_table$Rhizosphere<-as.factor(meco_datasetRZ$sample_table$Rhizosphere)
# Check
...
t1$cal_diff(method = "KW_dunn")
t1$data_stat
t1$plot_alpha(measure = "Observed", shape = "Rhizosphere")+labs(title="Fungi ASVs richness")
...
# t1 <- trans_alpha$new(dataset = meco_datasetRZ, group = "Rhizosphere")
# t1$cal_diff(method = "anova", formula = "Rhizosphere+Hill__side")
# head(t1$res_diff)
# see the help document for the usage of formula

#### Hills numbers calculation ####
phyloseq_inext(ps_rarefiedRhizosphere)
...
t1$cal_diff(method = "KW_dunn")
head(t1$res_diff)
t1$plot_alpha(measure = "Observed", shape = "Rhizosphere")+labs(title="Fungi Phylum richness")
...
# Acacia mangium 2-year-old
# remember first clone the whole dataset
group_AMY <- clone(meco_datasetRZ)
# select 'Two Years Old'
group_AMY$sample_table <- subset(group_AMY$sample_table, Rhizosphere == "Acacia mangium")
# trim all the data
group_AMY$tidy_dataset()
# Check
group_AMY
# Extract numeric parts (transect) and create grouping factor
group_AMY$sample_table$Numeric_Group <- as.numeric(stringr::str_extract(group_AMY$sample_table$original_sample_id, "(?<=\\D)(\\d+)"))
group_AMY$sample_table$Numeric_Group[is.na(group_AMY$sample_table$Numeric_Group)] <- 0
group_AMY$sample_table$Numeric_Group <- factor(group_AMY$sample_table$Numeric_Group)
# Check
group_AMY$sample_table$Numeric_Group
# Calculate alpha diversity at ASVs level
group_AMY<- trans_alpha$new(dataset = group_AMY, group = "Numeric_Group")
# See
group_AMY$data_alpha
# Do KW_dunn
group_AMY$cal_diff(method = "KW_dunn")
group_AMY$res_diff
# Plot
group_AMY$plot_alpha(measure = "Observed", shape = "Numeric_Group")

# Acacia mangium 10-year-old
# remember first clone the whole dataset
group_AMO <- clone(meco_datasetRZ)
# select 'Two Years Old'
group_AMO$sample_table <- subset(group_AMO$sample_table, Rhizosphere == "Acacia mangiumO")
# trim all the data
group_AMO$tidy_dataset()
# Check
group_AMO
# Extract numeric parts (transect) and create grouping factor
group_AMO$sample_table$Numeric_Group <- as.numeric(stringr::str_extract(group_AMO$sample_table$original_sample_id, "(?<=\\D)(\\d+)"))
group_AMO$sample_table$Numeric_Group[is.na(group_AMO$sample_table$Numeric_Group)] <- 0
group_AMO$sample_table$Numeric_Group <- factor(group_AMO$sample_table$Numeric_Group)
# Check
group_AMO$sample_table$Numeric_Group
# Calculate alpha diversity at ASVs level
group_AMO<- trans_alpha$new(dataset = group_AMO, group = "Numeric_Group")
# See
group_AMO$data_alpha
# Do KW_dunn
group_AMO$cal_diff(method = "KW_dunn")
group_AMO$res_diff
# Plot
group_AMO$plot_alpha(measure = "Observed", shape = "Numeric_Group")
...
#### Beta diversity calculation with Phylogenetic tree NOT FOR FUNGI WITH ITS2 and no additional information ####
...
# Phylogenetic tree construction. See https://adrientaudiere.github.io/MiscMetabar/reference/build_phytree_pq.html
 ...
 set.seed(22)
df <- subset_taxa_pq(ps_rarefiedRhizosphere, taxa_sums(ps_rarefiedRhizosphere) > 100)
...
# from phyloseq to phylo Phylogenetic tree
class(ps_tree)
my_phylo_object <- phy_tree(ps_tree)
my_phylo_object
class(my_phylo_object)
# Save unrooted tree
output_tree_file <- "extracted_phylo_treeFungiRZ.nwk"
write.tree(my_phylo_object, file = output_tree_file)
# Verify where saved
cat(paste0("Phylogenetic tree saved to: ", output_tree_file, "\n"))
# Root the tree with midpoint
rooted_ML_tree <- phangorn::midpoint(my_phylo_object)
# Get the names of the taxa (ASVs/OTUs) that are actually in phylogenetic tree.
# These are the tip labels of my_phylo_object.
taxa_in_tree <- tiplabels(rooted_ML_tree)
# Subset the otu_table of your meco_datasetRZ object. Make sure its rows (taxa) only contain those found in the tree.
# It's good practice to also ensure the order matches, although GUniFrac often handles this.
# Get current taxa in meco_datasetRZ's otu_table
taxa_in_meco_otu <- rownames(meco_datasetRZ$otu_table)
# Find common taxa (should be all taxa_in_tree if your previous steps are sound)
common_taxa <- intersect(taxa_in_tree, taxa_in_meco_otu)
# Check if all taxa from the tree are in the otu_table
if(length(setdiff(taxa_in_tree, taxa_in_meco_otu)) > 0) {
  warning("Not all taxa in the tree are present in the meco_datasetRZ's OTU table. Please investigate.")
}
# Check if all taxa from otu_table are in the tree
if(length(setdiff(taxa_in_meco_otu, taxa_in_tree)) > 0) {
  message("meco_datasetRZ's OTU table contains taxa NOT in the tree. These will be removed for UniFrac calculation.")
}
# Subset the meco_datasetRZ$otu_table to ONLY include taxa that are in the tree
# And make sure the order matches for robustness
meco_datasetRZSubset$otu_table <- meco_datasetRZ$otu_table[taxa_in_tree, ]
# Attach phylogenetic tree to microeco table (as before)
meco_datasetRZSubset$phylo_tree <- rooted_ML_tree
meco_datasetRZSubset$phylo_tree
# Calculation with Phylogenetic tree (should work now!)
meco_datasetRZSubset$cal_betadiv(unifrac = T)
#Attach phylogenetic tree to microeco table
meco_datasetRZSubset$phylo_tree<-rooted_ML_tree
meco_datasetRZSubset$phylo_tree
# Calculation with Phylogenetic tree, if unifrac=T it will use
# the phylogenetic tree, but note that the tree is built for the core community (taxa with>100 sequences)
meco_datasetRZ$cal_betadiv(unifrac = T)
...
plot<-t1$plot_ordination(plot_color = "Rhizosphere", plot_type = c("point","ellipse"), color_values = specific_colors)
...
t1$plot_ordination(plot_color = "Rhizosphere", point_size = 5, point_alpha = 2, plot_type = c("point", "chull"), ellipse_chull_fill = FALSE, ellipse_chull_alpha = 0.1)
t1$plot_ordination(plot_color = "Rhizosphere") + theme(panel.grid = element_blank()) + geom_vline(xintercept = 0, linetype = 2) + geom_hline(yintercept = 0, linetype = 2)
# PCA with Phylum data
tmp <- meco_datasetRZ$merge_taxa(taxa = "Phylum")
tmp$tax_table %<>% .[.$Phylum != "p__", ]
tmp$tidy_dataset()
rownames(tmp$otu_table) <- tmp$tax_table[rownames(tmp$otu_table), "Phylum"]
rownames(tmp$tax_table) <- tmp$tax_table[, "Phylum"]
# Plot and compare between distances
t1 <- trans_beta$new(dataset = tmp)
t1$cal_ordination(method = "PCA")
t1$res_ordination
...
t1 <- trans_abund$new(dataset = meco_datasetRZ, taxrank = "Genus", ntaxa = 15)
t1$plot_box(group = "Rhizosphere", xtext_angle = 30)
# show 15 taxa at ASVs level
t1 <- trans_abund$new(dataset = meco_datasetRZ, taxrank = "Species", ntaxa = 15)
t1$plot_box(group = "Rhizosphere", xtext_angle = 30)
# Heatmap
...
t1 <- trans_abund$new(dataset = meco_datasetRZ, taxrank = "Phylum", ntaxa = 4, groupmean = "Rhizosphere")
...
g2 <- t1$plot_diff_abund(select_taxa = t1$plot_diff_bar_taxa, plot_type = "barerrorbar", add_sig = FALSE, errorbar_addpoint = FALSE, errorbar_color_black = TRUE)
...
# select samples of "Acacia mangiumY" group
tmp <- clone(meco_datasetRZ)
tmp$sample_table %<>% subset(Rhizosphere == "Acacia mangiumY")
tmp$tidy_dataset()
tmp <- trans_network$new(dataset = tmp, cor_method = "spearman", filter_thres = 0.0005)
tmp$cal_network(COR_p_thres = 0.01, COR_cut = 0.6)
soil_amp_network$AcaciaY <- tmp
# select samples of "Acacia mangiumO" group
tmp <- clone(meco_datasetRZ)
tmp$sample_table %<>% subset(Rhizosphere == "Acacia mangiumO")
tmp$tidy_dataset()
tmp <- trans_network$new(dataset = tmp, cor_method = "spearman", filter_thres = 0.0005)
tmp$cal_network(COR_p_thres = 0.01, COR_cut = 0.6)
soil_amp_network$AcaciaO <- tmp
...
Metadata_RZ<-Metadata[Metadata$ProjectFocus=="Rhizosphere",]
Metadata_RZ$Rhizosphere<-as.factor(Metadata_RZ$Rhizosphere)
Metadata_RZ[,22:31]
# Create the object and complete_na= True because 0 and NA 
t1 <- trans_env$new(dataset = tmp_mt, add_data = Metadata_RZ[,22:31],complete_na = TRUE)
# Re-check the levels after creating t1
table(t1$dataset$sample_table$Rhizosphere)
# Difference across groups with Wilcoxon
...
t1$cal_autocor()
# Correlation for different Rhizosphere
t1$cal_autocor(group = "Rhizosphere")

# Do dbRDA ordination
# use jaccard distance for dbRDA
t1$cal_ordination(method = "dbRDA", use_measure = "jaccard", group="Rhizosphere")
...
# The result says 100% on X axis because I used average values for all variables except for SOC.
# This means that I have equal values and this impedes the calculations.
...
t12$plot_ordination(plot_color = "Rhizosphere",plot_type = c("point"))+theme_bw()+ggtitle("Functional Fungi Rhizosphere")
t12$res_ordination
...
Metadata_field<-Metadata[Metadata$ProjectFocus=="Rhizosphere",]
...
#### Differential test to see different functions across groups, Rhizosphere ####
# First, it is better to clone the dataset
tmp_mt <- clone(meco_datasetRZ)
...
t2 <- trans_diff$new(dataset = tmp_mt, method = "KW_dunn", group = "Rhizosphere", p_adjust_method="none", taxa_level = "func")
# See results of relative abundance
t2$res_abund
write.csv(t2$res_abund, file = "FungiFunctAbundance_table.csv")
# See KW_dunn results
t2$res_diff
