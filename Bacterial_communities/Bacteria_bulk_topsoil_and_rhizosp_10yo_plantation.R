...
otu_dataBulk <- read.csv("features_table10yoBacteria.csv")
nrow(otu_dataBulk)
...
# Try KW_dunn
t1$cal_diff(method = "KW_dunn")
t1$plot_alpha(measure = "Observed", shape = "ProjectFocus")+labs(title="Bacteria ASVs richness 10yo")
...
t1$cal_diff(method = "KW_dunn")
head(t1$res_diff)
t1$plot_alpha(measure = "Observed", shape = "ProjectFocus")+labs(title="Bacteria Phylum richness 10yo")
...
plot + labs(title="Bacteria communities 10yo")+theme_bw()
...
plot + labs(title="Bacteria communities 10yo")+theme_bw()+theme(axis.title = element_text(size = 16),axis.text.x = element_text(size = 14), 
...
custom_phylum_colors <- c(
  "Actinomycetota" = "pink",
  "Planctomycetota" = "steelblue",
  "Verrucomicrobiota" = "forestgreen",
  "Pseudomonadota"="lightblue",
  "Acidobacteriota" = "orange",
  "Others" = "grey70"
)
t1$plot_bar(color_values=custom_phylum_colors,others_color = "grey70", facet = "ProjectFocus", xtext_keep = FALSE, legend_text_italic = FALSE)+
  labs(title = "Relative abundance of bacteria phyla Bulk-Rhizo 10yo")
...
t1$plot_bar(others_color = "grey70", facet = "ProjectFocus", xtext_keep = FALSE, legend_text_italic = FALSE)+
  labs(title = "Relative abundance of bacteria ASVs Bulk-Rhizo 10yo")
...
t1$res_abund
t1$plot_diff_abund()+
  labs(title = "Relative abundance of bacteria phyla Bulk-Rhizo 10yo with KW")
t1$res_diff
...
# Save the result
write.table(t2$res_spe_func_perc, "FunctionBacteriaBulkRhizo10YO.csv", row.names = TRUE)
...
t9<-read.csv("FunctionBacteria_MultiPlotBulkRhizo10YO.csv", row.names=1)
...
t12$plot_ordination(plot_color = "ProjectFocus",plot_type = c("point"))+theme_bw()+ggtitle("Functional Bacteria Bulk and Rhizosphere 10yo")
t12$res_ordination
...
t2$res_abund
write.csv(t2$res_abund, file = "BacteriaFunctAbundanceBulkRhizo10yo_table.csv")
# See KW_dunn results
t2$res_diff
write.csv(t2$res_diff, file = "KW_Functions_abundance_tableBulkRhizo10yo.csv")
t2$plot_diff_abund(add_sig = TRUE, use_number = 1:9) + ggplot2::ylab("Relative abundance (%)")+ggplot2::labs(title = "Bacteria functional groups Bulk-Rhizo 10yo")
