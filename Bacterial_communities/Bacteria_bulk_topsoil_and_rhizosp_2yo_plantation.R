...
otu_dataBulk <- read.csv("features_table2yoBacteria.csv") # Check that there is the ID column and that the samples don't have the hyphen but the dot
nrow(otu_dataBulk)
...
# Filtered the taxa not present in the subset (2yo)
...
t1$res_ordination
# plot the PCoA result without confidence ellipse
specific_colors <- c("Field"= "#CDC9A5","Saccharum spp." = "darkorchid1", "Acacia mangiumY" = "deepskyblue1", 
                     "Coccus nucifera" = "chocolate4", "Imperata cylindrica"="red")
plot<-t1$plot_ordination(plot_color = "ProjectFocus", plot_type = "point", color_values = specific_colors, point_size=4)
plot + labs(title="Bacteria communities 2yo")+theme_bw()+theme(axis.title = element_text(size = 16),axis.text.x = element_text(size = 14), 
...
t1$plot_bar(color_values=custom_phylum_colors,others_color = "grey70", facet = "ProjectFocus", xtext_keep = FALSE, legend_text_italic = FALSE)+
  labs(title = "Relative abundance of bacteria phyla Bulk-Rhizo 2yo")
...
t1$plot_bar(others_color = "grey70", facet = "ProjectFocus", xtext_keep = FALSE, legend_text_italic = FALSE)+
  labs(title = "Relative abundance of bacteria ASVs Bulk-Rhizo 2yo")
...
t1$res_abund
t1$plot_diff_abund()+
  labs(title = "Relative abundance of bacteria phyla Bulk-Rhizo 2yo with KW")
t1$res_diff
...
# Save the result
write.table(t2$res_spe_func_perc, "FunctionBacteriaBulkRhizo2YO.csv", row.names = TRUE)
...
t9<-read.csv("FunctionBacteria_MultiPlotBulkRhizo2yo.csv", row.names=1)
...
t2$res_abund
write.csv(t2$res_abund, file = "BacteriaFunctAbundanceBulkRhizo2yo_table.csv")
# See KW_dunn results
t2$res_diff
write.csv(t2$res_diff, file = "KW_Functions_abundance_tableBulkRhizo2yo.csv")
t2$plot_diff_abund(add_sig = TRUE, use_number = 1:9) + ggplot2::ylab("Relative abundance (%)")+ggplot2::labs(title = "Bacteria functional groups Bulk-Rhizo 2yo")
