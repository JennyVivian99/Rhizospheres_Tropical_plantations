...
specific_colors <- c("Field2"= "#CDC9A5","Saccharum spp." = "darkorchid1", "Acacia mangiumY" = "deepskyblue1", 
                     "Coccus nucifera" = "chocolate4", "Imperata cylindrica"="red","Field10"= "#CDC9A5","Acacia mangiumO" = "deepskyblue1", "Mahogany (Swietenia macrophylla)" = "deepskyblue1", 
...
plot + labs(title="Fungi communities 2 and 10yo")+theme_bw()+theme(axis.title = element_text(size = 16),axis.text.x = element_text(size = 14), 
                                                            # Control the size of the tick mark labels on the Y-axis
                                                            axis.text.y = element_text(size = 14))+ shape = specific_shapes)

# Plot with centroids
plot <- t1$plot_ordination(
  plot_color = "ProjectFocus", 
  plot_shape = "ProjectFocus",
  plot_type = c("point", "centroid"), 
  ellipse_type = "norm",           # Switch from "t" to "norm"
  color_values = specific_colors, 
  shape_values = specific_shapes,  # Map shapes here to avoid warnings
  point_size = 4
)
plot + 
  labs(title = "Fungi communities 2 and 10yo") +
  theme_bw() +
  theme(
...
t1 <- trans_beta$new(dataset = dataset, group = "ProjectFocus", measure = "jaccard")
t1$cal_manova(manova_all = F, p_adjust_method = "fdr") # with adjusted values
t1$res_manova
write.csv(t1$res_manova,"permanovaITSRhizoBulk.csv",row.names = T)
...
t1$res_betadisper
# Extract the table
permdisp_tab <- as.data.frame(t1$res_betadisper$pairwise$permuted)
# Save it
write.csv(permdisp_tab, "permdispITSBulkRhizoresults.csv")
